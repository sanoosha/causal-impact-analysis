...
SELECT 
  ...
  , avg(level_starts_per_day) AS level_starts_per_day
  , sum(level_starts)/count(distinct user_pseudo_id) AS level_starts_per_user
  , avg(sessions) AS sessions
  , avg(sessions_per_day) AS sessions_per_day
FROM pre_post
GROUP BY 1, 2